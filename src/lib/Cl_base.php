...
class Cl_base{
    ...
    public function bind($parametro, $valor, $tipo = null){
        ...
        $this->at_sql->bindValue($parametro, $valor, $tipo);
    ...
    public function makeQuery($sql, $params, $insert = False){
        $this->sql($sql);
        foreach($params as $key => $param){
            $this->bind($key+1, $param);
        }
        if($insert){
            return $this->execute();
        }else{
    ...
    public function makeInserts($sql, $params){
        return $this->makeQuery($sql, $params, True);
    }
